...
-- -------- guest-safe, server-priced order creation --------
...
-- returns the new id + order_number + authoritative total.
--
-- SECURITY: prices are recomputed here from the `meals` table by slug +
-- bundle_type — the client-sent p_subtotal/p_total are IGNORED, so a tampered
-- request can't underpay. Custom "build your own" meals have no catalog row,
-- so their client-composed price is accepted but clamped to a sane ceiling.
-- (Kept in sync with supabase/2026-06-place-order-rpc.sql — run that file to
...
values
  ...
  ('honey-salmon',
   ...
   'assets/meals/honey-salmon.webp',
   540, 42, 44, 16,
   9.00, 40.50, 77.00,
  ...
  ('halloumi-couscous',
   ...
   'assets/meals/halloumi-couscous.webp',
   510, 26, 48, 22,
   7.00, 31.50, 60.00,
  ...
  ('tikka-chicken',
   ...
   'assets/meals/tikka-chicken.webp',
   395, 46, 14, 15,
   7.50, 33.50, 64.00,
